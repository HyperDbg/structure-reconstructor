...
fn print_structure(index: u32) {
    ...
    unsafe {
        ...
        for item in &MY_VECTOR {
            if item.index != current_index || first_index {
                ...
                if item.size == 64 {
                    output.push_str(&format!("long long Field{}", field_index));
                } else if item.size == 32 {
                    output.push_str(&format!("int Field{}", field_index));
                } else if item.size == 8 {
                    output.push_str(&format!("char Field{}", field_index));
                } else {
                    output.push_str(&format!("unknown Field{}", field_index));
                }
...
fn zydis_disasm_interpret(code: &[u8],
    ...
    for insn_info in dec.decode_all::<VisibleOperands>(code, rip) {
        ...
        let offset = if context >= base {
                (context - base) as u32
            } else {
                (context - 0) as u32
        };

        add_to_vector_if_not_exists(rip, context, offset, insn.operand_width);
...
fn parse_hex_to_u64(hex_str: &str) -> std::result::Result<u64, ParseIntError> {
 ...
 fn main() -> io::Result<()>  {
    ...
    //
    // Attempt to delete "structs.h" at the start of the program
    //
    if let Err(e) = fs::remove_file("structs.h") {

        //
        // If the error is not due to the file not existing, print the error
        //
        if e.kind() != std::io::ErrorKind::NotFound {
            eprintln!("Failed to delete structs.h: {}", e);
        }
    }
...
//
// cargo run -- fffff28d1d3c6000 "C:\Users\sina\Desktop\stack-test.txt"
//
